...
- `dataset/masked_inpaint_shape_refinement_v1`: 3 train / 1 val. This is the first approved `shape_refinement` side-route subset (`pair_0074`, `pair_0100`, `pair_0209` train; `pair_0073` val).
- `dataset/masked_inpaint_shape_refinement_v2`: 5 train / 1 val. This is the older partial second `shape_refinement` approved subset that merged the original v1-approved set with the first clean v2 seed passes.
- `dataset/masked_inpaint_shape_refinement_v3`: 6 train / 2 val. This is the current merged `shape_refinement` approved subset after replacing the retired offset-heavy tail (`pair_0120`, `pair_0043`) with the new clean top-off masks (`pair_0208` train; `pair_0022` val).
- `dataset/masked_inpaint_shape_refinement_v4`: 10 train / 3 val. This is the current merged `shape_refinement` approved subset after promoting the clean `v4` seed pack (`pair_0028`, `pair_0057`, `pair_0106`, `pair_0158` train; `pair_0035` val).
...
Current shape-refinement side-route:

- Active merged manifest: [`dataset/annotations/masked_shape_refinement_v3_approved_subset_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v3_approved_subset_manifest.json)
- Active merged dataset: [`dataset/masked_inpaint_shape_refinement_v3`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/masked_inpaint_shape_refinement_v3)
- Local smoke: [`/tmp/masked_inpaint_lora_shape_refinement_v3_local_smoke`](/tmp/masked_inpaint_lora_shape_refinement_v3_local_smoke)
- Local short dry-run: [`/tmp/masked_inpaint_lora_shape_refinement_v3_step10_local`](/tmp/masked_inpaint_lora_shape_refinement_v3_step10_local)
- Next GPU handoff config: [`colab/masked_inpaint_shape_refinement_v3_pilot.yaml`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/colab/masked_inpaint_shape_refinement_v3_pilot.yaml)
...
### Experiment 2026-04-24A - Promote The Clean V4 Shape-Refinement Seed Into The Merged Side Route

Hypothesis:
If the newly drawn `v4` seed masks stay narrow and shape-driven, they should be safe to merge into the current `shape_refinement` side-route without reintroducing the offset-heavy noise that forced `pair_0120` and `pair_0043` out.

Change made:

- Reviewed the uploaded `v4` masks for:
  - `pair_0028`
  - `pair_0057`
  - `pair_0106`
  - `pair_0158`
  - `pair_0035`
- Promoted the full clean tranche into:
...
- Rebuilt the merged side-route dataset into:
...
- All five `v4` masks pass first-round QA.
- The merged `shape_refinement` dataset now contains:
  - `train_count = 10`
  - `val_count = 3`
- Dataset summary remains healthy:
...
- The `shape_refinement` side-route now has enough clean incremental coverage to justify another GPU-side pilot.
- The next valuable manual boundary is no longer mask QA; it is running the `v4` Colab pilot.

### Experiment 2026-04-24B - Rebuild Local Handoff Around Shape-Refinement V4

Hypothesis:
If the merged `v4` side-route dataset is structurally sound, then the existing training stack should at least pass a local smoke check and be ready for a refreshed Colab handoff.

Change made:

- Added [`colab/masked_inpaint_shape_refinement_v4_pilot.yaml`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/colab/masked_inpaint_shape_refinement_v4_pilot.yaml)
- Updated [`colab/train_masked_inpaint_full12_v1.ipynb`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/colab/train_masked_inpaint_full12_v1.ipynb) so a fresh clone now defaults to the `v4` pilot config
- Ran a local smoke on the merged dataset:
...
- The local `10`-step dry-run was launched but remains too slow on this CPU to use as a blocking gate for the next move.
- The refreshed Colab handoff is now defined against:
  - dataset: `dataset/masked_inpaint_shape_refinement_v4`
  - manifest: `dataset/annotations/masked_shape_refinement_v4_approved_subset_manifest.json`
  - Drive dataset dir: `/content/drive/MyDrive/masked_inpaint_shape_refinement_v4`
  - Drive output dir: `/content/drive/MyDrive/nail-retouch-masked-shape-refinement-v4-pilot-outputs`

Conclusion:

- `shape_refinement_v4` is structurally ready for the next GPU-side pilot.
- Waiting for a CPU-bound local `10`-step run is lower-value than moving to Colab now.

### Experiment 2026-04-18A - Archive And Evaluate The Full12 Lambda-Identity-7p5 Run

Hypothesis:
If the current masked reference is under-regularizing preserve-region behavior, a moderate increase in `lambda_identity` from `5.0` to `7.5` should improve unmasked-region preservation without materially hurting edit quality.

Change made:

- Archived the user-provided zip into [`archive/2026-04-06_user_result_zips/nail-retouch-masked-full12-lambda-identity-7p5-outputs-20260418T025639Z-3-001.zip`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/archive/2026-04-06_user_result_zips/nail-retouch-masked-full12-lambda-identity-7p5-outputs-20260418T025639Z-3-001.zip)
- Extracted the run into [`outputs/masked_inpaint_colab_runs/full12_lambda_identity_7p5_run_2026-04-17_step150/nail-retouch-masked-full12-lambda-identity-7p5-outputs`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/outputs/masked_inpaint_colab_runs/full12_lambda_identity_7p5_run_2026-04-17_step150/nail-retouch-masked-full12-lambda-identity-7p5-outputs)
...
- Best checkpoint inside the run is `step150`
- New `step150` summary:
  - `masked_l1_to_target = 0.0653356`
  - `masked_delta_e_to_target = 8.5277605`
  - `unmasked_l1_to_input = 0.0056021`
  - `unmasked_delta_e_to_input = 1.1304536`
  - `border_l1_to_target = 0.0361828`
- Current masked reference summary:
  - `masked_l1_to_target = 0.0653309`
  - `masked_delta_e_to_target = 8.5266851`
  - `unmasked_l1_to_input = 0.0056065`
...
- `lambda_identity = 7.5` is safe but effectively flat relative to the current masked reference.
- It provides a tiny preservation gain, but the edit-side metrics become very slightly worse and the total difference is too small to justify a checkpoint promotion.
- The current masked default should remain the `full12 lambda_color=1.0 step150` checkpoint.

### Experiment 2026-04-18B - Choose And Prepare The Next Masked Single-Variable Handoff

Hypothesis:
After `v3 dataset-only` and `lambda_identity=7.5` both landed in a flat regime, the next most plausible frontier-moving lever is a small additional increase in inside-mask color pressure rather than another preservation or dataset-only change.

Change made:

- Reused evaluation and training roles to propose the next single-variable move:
  - evaluation recommendation: `lambda_color = 1.25`
  - training recommendation: `rank = 8`
- Chose the `lambda_color = 1.25` route because it follows the only recent variable family that has already produced a real improvement in-project
- Added [`colab/masked_inpaint_full12_lambda_color_1p25_v1.yaml`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/colab/masked_inpaint_full12_lambda_color_1p25_v1.yaml)
- Updated [`colab/train_masked_inpaint_full12_v1.ipynb`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/colab/train_masked_inpaint_full12_v1.ipynb) so a fresh clone now defaults to the new `lambda_color=1.25` config
...
### Experiment 2026-04-18E - Archive And Evaluate The Full12 Rank8 Run

Hypothesis:
If the current masked route is bottlenecked by LoRA capacity, increasing `rank` from `4` to `8` should improve the patched 4-anchor validation means relative to the current full12 masked reference.

Change made:

- Archived the user-provided zip into [`archive/2026-04-06_user_result_zips/nail-retouch-masked-full12-rank8-outputs-20260418T173352Z-3-001.zip`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/archive/2026-04-06_user_result_zips/nail-retouch-masked-full12-rank8-outputs-20260418T173352Z-3-001.zip)
- Extracted the run into [`outputs/masked_inpaint_colab_runs/full12_rank8_run_2026-04-18_step150/nail-retouch-masked-full12-rank8-outputs`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/outputs/masked_inpaint_colab_runs/full12_rank8_run_2026-04-18_step150/nail-retouch-masked-full12-rank8-outputs)
- Evaluated `step100 / 125 / 150` on the same patched 4-anchor validation protocol used for the current masked reference
...
- The run is complete and internally consistent:
  - checkpoints at `25 / 50 / 75 / 100 / 125 / 150`
  - stable enough to evaluate, but noisier than the current reference
- Best checkpoint inside the run is `step150`
- New `step150` summary:
  - `masked_l1_to_target = 0.0656602`
  - `masked_delta_e_to_target = 8.5940570`
  - `unmasked_l1_to_input = 0.0056190`
  - `unmasked_delta_e_to_input = 1.1351714`
  - `border_l1_to_target = 0.0362734`
- Current masked reference summary:
  - `masked_l1_to_target = 0.0653309`
  - `masked_delta_e_to_target = 8.5266851`
  - `unmasked_l1_to_input = 0.0056065`
...
- `rank 8` is a clear backward move relative to the current masked reference.
- The regression is not catastrophic, but it is directionally consistent across edit, preserve, and border metrics.
- This adds evidence that the current bottleneck is not simply insufficient LoRA capacity at `rank 4`.

### Experiment 2026-04-18F - Split Out A Tiny Shape-Refinement Side Seed

Hypothesis:
If the current masked plateau is caused in part by mixing broader contour/geometry edits into the local-boundary task family, then a tiny explicitly separate `shape_refinement` side-seed should have higher information gain than more nearby hyperparameter nudges.

Change made:

- Reused evaluation and dataset roles to rank next-step data/task-structure options
- Chose an explicit taxonomy split over more conservative `v3` local-boundary expansion
- Created a tiny `shape_refinement` side-seed scaffold with:
  - [`dataset/annotations/masked_shape_refinement_v1_seed_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v1_seed_manifest.json)
  - [`dataset/annotations/masked_shape_refinement_v1_seed_pack_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v1_seed_pack_manifest.json)
  - [`dataset/annotation_packs/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotation_packs/masked_shape_refinement_v1_seed)
  - [`dataset/annotations/masks/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v1_seed)

Result:

- First-pass seed uses:
  - train: `pair_0073`, `pair_0074`, `pair_0100`, `pair_0209`
  - val: `pair_0120`
- `pair_0208` was intentionally excluded from the first pass because it currently looks more like a high-noise / higher-risk candidate than a clean first signal for the new task bucket

Conclusion:

- The project now has a concrete side-route scaffold for `shape_refinement`.
- This keeps the stable `proximal_nail_boundary_refinement` mainline clean while giving us a direct way to test whether task-mixture is the real bottleneck.

### Experiment 2026-04-19A - Remove `pair_0120` From The First Shape-Refinement Pass

Hypothesis:
If `pair_0120` carries severe before/after geometric offset, keeping it in the very first `shape_refinement` pass would add pairing noise faster than it adds useful task signal.

Change made:

- Updated [`dataset/annotations/masked_shape_refinement_v1_seed_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v1_seed_manifest.json)
- Updated [`dataset/annotations/masked_shape_refinement_v1_seed_pack_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v1_seed_pack_manifest.json)
- Removed `pair_0120` from the first-pass active split
- Promoted `pair_0073` into the first validation slot instead

Result:

- Current first-pass `shape_refinement` seed is now:
  - train: `pair_0074`, `pair_0100`, `pair_0209`
  - val: `pair_0073`
- `pair_0120` remains an observed high-risk candidate, but it is no longer a required blocker for the first side-route pass

Conclusion:

- The first `shape_refinement` pass is now cleaner and less likely to be dominated by pairwise geometry noise.
- `pair_0120` should be treated as a later hard-case candidate, not as a first-pass readout anchor.

### Experiment 2026-04-19B - Run First-Pass QA On The Uploaded Shape-Refinement Seed Masks

Hypothesis:
If the first `shape_refinement` masks are semantically local and follow real contour/tip-corridor changes rather than broad hand edits, at least part of the side-seed should already be promotable or close to promotable.

Change made:

- Reviewed the uploaded masks under [`dataset/annotations/masks/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v1_seed)
- Compared them against the context sheets in [`dataset/annotation_packs/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotation_packs/masked_shape_refinement_v1_seed)
- Ran first-pass Mask QA on:
  - `pair_0073`
  - `pair_0074`
  - `pair_0100`
  - `pair_0209`
...
- `pair_0073`: pass
  - approx mask ratio `0.0461`
- `pair_0074`: micro-adjust
  - approx mask ratio `0.0496`
  - needs slightly more distal sidewall/tip-transition coverage
- `pair_0100`: micro-adjust
  - approx mask ratio `0.0346`
  - currently undercovers sharper distal/tip silhouette changes
- `pair_0209`: micro-adjust
  - approx mask ratio `0.0272`
  - currently a little too conservative for a stiletto-shape example

Conclusion:

- The first shape side-seed is viable: no sample needs to be discarded or deferred.
- But only `pair_0073` is ready for immediate promotion; the other three need one refinement pass before a clean first approved subset should be built.

### Experiment 2026-04-19C - Re-Run QA After The First Shape-Refinement Micro-Adjust Pass

Hypothesis:
If the uploaded revisions on `pair_0074`, `pair_0100`, and `pair_0209` meaningfully expand the distal sidewall / tip corridor without broadening into hand-wide edits, the first `shape_refinement` seed should be ready for approved-subset promotion.

Change made:

- Re-reviewed the latest masks under [`dataset/annotations/masks/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v1_seed)
- Compared the revised masks against the same context sheets in [`dataset/annotation_packs/masked_shape_refinement_v1_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotation_packs/masked_shape_refinement_v1_seed)
- Focused the second-pass QA on:
  - `pair_0074`
  - `pair_0100`
  - `pair_0209`
...
- `pair_0073`: still passes cleanly
  - approx mask ratio `0.0461`
- `pair_0074`: still needs micro-adjust
  - approx mask ratio `0.0496`
  - remaining issue: still slightly proximal-heavy and still wants fuller distal sidewall / tip-corridor coverage on the dominant long french nails
- `pair_0100`: still needs micro-adjust
  - approx mask ratio `0.0346`
  - remaining issue: still undercovers the sharper distal silhouette / tip-transition region where the visible shape correction happens
- `pair_0209`: still needs micro-adjust
  - approx mask ratio `0.0272`
  - remaining issue: still a bit too conservative for the stiletto-shape example and needs slightly more distal sidewall / tip corridor coverage

Conclusion:

- The first `shape_refinement` side-seed remains structurally sound, but it is not yet ready for approved-subset promotion.
- `pair_0073` stays as the only current pass.
- `pair_0074`, `pair_0100`, and `pair_0209` need one more targeted refinement pass; none of them need to be discarded or deferred.

### Experiment 2026-04-19D - Check The Latest Single-Pair Fixes On The Shape-Refinement Seed

Hypothesis:
If the latest pair-by-pair fixes really complete the intended distal sidewall / tip corridor, the first `shape_refinement` seed can be reduced to at most one remaining blocker before promotion.

Change made:

- Re-checked the latest authored masks against their sheets for:
  - `pair_0209`
  - `pair_0100`
  - `pair_0074`
...
- `pair_0209`: now passes
  - the visible stiletto tip corridor is now covered cleanly without broadening into the nail interior or surrounding skin
- `pair_0100`: now passes
  - the dominant nails now include enough distal silhouette / tip-transition coverage to read as true `shape_refinement`
- `pair_0074`: still needs a micro-adjust
  - approximate mask ratio: `0.0527`
  - remaining issue: the main visible long french nails are better than before, but the distal sidewall / tip corridor is still slightly too narrow, especially on the right-side visible nail

Conclusion:

- The first `shape_refinement` seed is now very close to promotable.
- Current pass set:
  - `pair_0073`
  - `pair_0100`
  - `pair_0209`
- Only `pair_0074` remains as the last blocker before approved-subset promotion.

### Experiment 2026-04-19E - Promote The First Shape-Refinement Approved Subset And Run Minimal Training Closure

Hypothesis:
If the final blocker `pair_0074` now passes, the first `shape_refinement` seed can be promoted into a small approved subset that should still build cleanly and complete the same minimal local training closure used on the main masked route.

Change made:

- Final QA confirmed that the first `shape_refinement` seed now fully passes:
  - `pair_0073`
  - `pair_0074`
  - `pair_0100`
  - `pair_0209`
- Added approved manifest:
...
- Built the first dedicated shape dataset:
...
- Approved subset split:
  ...
  - val:
    - `pair_0073`
- Data-layer summary for [`dataset/masked_inpaint_shape_refinement_v1`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/masked_inpaint_shape_refinement_v1):
  - `train_count = 3`
  - `val_count = 1`
  - train `mean_mask_ratio = 0.0454`
  - val `mean_mask_ratio = 0.0470`
  - train `mean_final_luma_delta = 0.00107`
  - val `mean_final_luma_delta = 0.00276`
- Local smoke completed and wrote:
  - `metrics.jsonl`
  - `training_config.json`
  - `lora_checkpoints/pytorch_lora_weights_step_000004.safetensors`
  - `previews/preview_step_000004.png`
- Short dry-run completed and wrote:
  - `metrics.jsonl`
  - `training_config.json`
  - `lora_checkpoints/pytorch_lora_weights_step_000010.safetensors`
  - `previews/preview_step_000010.png`
- Losses stayed finite through both local runs.

Conclusion:

- The first `shape_refinement` side-route has now cleared:
  - mask QA
  - approved-subset promotion
  - dataset build
  - local smoke
  - short dry-run
- The project now has a minimal but real trainable side-route for stronger contour edits, separate from the `proximal_nail_boundary_refinement` mainline.

### Experiment 2026-04-20A - Prepare The First GPU-Side Shape-Refinement Pilot

Hypothesis:
If the first approved `shape_refinement` subset is already build-safe and trainer-safe locally, then the next highest-information move is a small GPU-side pilot that keeps the masked mainline hyperparameters fixed and only tests whether this side route learns coherent signal at all.

Change made:

- Added first pilot config:
...
- Updated the shared masked Colab notebook default:
...
- The side-route now has a direct Colab handoff.
- The intended Drive dataset path is:
  - `/content/drive/MyDrive/masked_inpaint_shape_refinement_v1`
- The intended Drive output path is:
  - `/content/drive/MyDrive/nail-retouch-masked-shape-refinement-v1-pilot-outputs`
- Core masked variables stay unchanged for interpretability:
  - `resolution = 512`
  - `rank = 4`
  - `learning_rate = 1e-5`
  - `lambda_identity = 5.0`
...
- The first `shape_refinement` side-route is now ready for its first GPU-side readability run.
- The project is back at a clean manual boundary: upload the built side-route dataset to Drive and run the pilot from a fresh Colab clone.

### Experiment 2026-04-20B - Archive And Evaluate The First Shape-Refinement GPU Pilot

Hypothesis:
If `shape_refinement` is a real trainable side route rather than only an annotation split, the first GPU-side pilot should train stably, produce a readable checkpoint ladder, and reveal whether the next bottleneck is more steps or more data.

Change made:

- Archived the user-provided zip into [`archive/2026-04-06_user_result_zips/nail-retouch-masked-shape-refinement-v1-pilot-outputs-20260420T054508Z-3-001.zip`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/archive/2026-04-06_user_result_zips/nail-retouch-masked-shape-refinement-v1-pilot-outputs-20260420T054508Z-3-001.zip)
- Extracted the run into [`outputs/masked_inpaint_colab_runs/shape_refinement_v1_pilot_run_2026-04-20_step100/nail-retouch-masked-shape-refinement-v1-pilot-outputs`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/outputs/masked_inpaint_colab_runs/shape_refinement_v1_pilot_run_2026-04-20_step100/nail-retouch-masked-shape-refinement-v1-pilot-outputs)
- Reviewed checkpoints, previews, metrics, and training config for the `25 / 50 / 75 / 100` ladder

Result:

- The run is stable and complete:
  - checkpoints at `25 / 50 / 75 / 100`
  - previews at `25 / 50 / 75 / 100`
  - no collapse or obvious late runaway
- Metrics trend confirms that the route learns something coherent:
  - windowed loss average improves from roughly `9.52` in `1-25` to `4.25` in `76-100`
- Best checkpoint appears to be `step75`, with `step50` as the nearest fallback
- `step100` looks slightly worse than `step75`, suggesting a tiny-set late-run wobble rather than a need for more steps

Conclusion:

- The first GPU-side `shape_refinement` pilot validates the route: it is trainable and produces a meaningful early checkpoint ladder.
- The next bottleneck is more data, not more steps.
- The next useful action is to expand the `shape_refinement` annotation pool rather than running a longer pilot on the current `3 train / 1 val` subset.

### Experiment 2026-04-20C - Prepare The Next Conservative Shape-Refinement Seed Pack

Hypothesis:
If the first `shape_refinement` pilot is stable but sample-starved, then the next highest-information move is a small second seed pack that adds one hard geometric probe plus a few cleaner same-family contour examples.

Change made:

- Prepared a new annotation pack:
  - [`dataset/annotation_packs/masked_shape_refinement_v2_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotation_packs/masked_shape_refinement_v2_seed)
- Added pack manifest:
  - [`dataset/annotations/masked_shape_refinement_v2_seed_pack_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v2_seed_pack_manifest.json)
- Added seed manifest:
  - [`dataset/annotations/masked_shape_refinement_v2_seed_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v2_seed_manifest.json)
- Reserved final mask root:
  - [`dataset/annotations/masks/masked_shape_refinement_v2_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v2_seed)

Result:

- Current second seed split:
  - train:
    - `pair_0120`
    - `pair_0071`
    - `pair_0066`
  - val:
    - `pair_0043`
- `pair_0120` is intentionally included as the high-information geometric probe, but the pack README now explicitly warns that its offset should not be followed into a broad mask.
- `pair_0208` remains out of this more conservative second seed.

Conclusion:

- The next manual boundary is now clear and ready to use: annotate the `shape_refinement_v2_seed` pack.
- This keeps the side route moving without overcommitting to the noisiest hard-case pool.

### Experiment 2026-04-22A - QA The Second Shape-Refinement Seed And Promote The Passed Partial Subset

Hypothesis:
If the second `shape_refinement` seed contains a mix of clearly usable and still-noisy masks, then promoting only the cleanly passing members should increase side-route dataset size without waiting for every hard sample to clear QA.

Change made:

- Ran first-pass QA on the uploaded v2 seed masks in [`dataset/annotations/masks/masked_shape_refinement_v2_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v2_seed)
- Classified:
  - `pair_0071`: pass
  - `pair_0066`: pass
  - `pair_0120`: micro-adjust
  - `pair_0043`: micro-adjust
- Built a merged approved-subset manifest:
...
- Rebuilt the merged side-route dataset:
...
- Ran local training closure checks on the merged subset:
  - smoke: [`/tmp/masked_inpaint_lora_shape_refinement_v2_local_smoke`](/tmp/masked_inpaint_lora_shape_refinement_v2_local_smoke)
  - short dry-run: [`/tmp/masked_inpaint_lora_shape_refinement_v2_step10_local`](/tmp/masked_inpaint_lora_shape_refinement_v2_step10_local)

Result:

- The new approved-subset manifest now contains:
  - train:
    - `pair_0074`
    - `pair_0100`
    - `pair_0209`
...
- Rebuilt dataset summary:
  - `train_count = 5`
  - `val_count = 1`
  - train `mean_mask_ratio = 0.0443`
  - val `mean_mask_ratio = 0.0470`
  - train `mean_final_luma_delta = 0.00154`
  - val `mean_final_luma_delta = 0.00276`
- The merged subset completes both local smoke and 10-step dry-run checks without trainer regressions.
- Remaining manual blockers are now only:
  - `pair_0120`
  - `pair_0043`

Conclusion:

- The second `shape_refinement` seed is already partially useful even before the two hardest samples are fixed.
- `pair_0071` and `pair_0066` should remain promoted in the current merged side-route subset.
- The next manual boundary is now narrower and more concrete: only `pair_0120` and `pair_0043` still need mask revision before the next merged rebuild.

### Experiment 2026-04-22B - Retire The Offset-Prone V2 Tail And Replace It With A Smaller Shape-Refinement Top-Off Pack

Hypothesis:
If `pair_0120` and `pair_0043` are both now judged too offset-prone to be worth salvaging, replacing them with a tiny new top-off pack should be safer than continuing to spend annotation effort on noisy pairs.

Change made:

- Accepted the user decision to stop using `pair_0120` and `pair_0043` for the active shape side-route.
- Generated a replacement annotation pack:
...
- Added replacement manifests:
  - [`dataset/annotations/masked_shape_refinement_v3_seed_pack_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v3_seed_pack_manifest.json)
  - [`dataset/annotations/masked_shape_refinement_v3_seed_manifest.json`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masked_shape_refinement_v3_seed_manifest.json)
- Reserved the new final mask root:
  - [`dataset/annotations/masks/masked_shape_refinement_v3_seed`](/Volumes/DevSSD/AI-projects/nail-retouch-assistant/dataset/annotations/masks/masked_shape_refinement_v3_seed)
- Selected the replacement pair set:
  - train: `pair_0208`
  - val: `pair_0022`
- Rewrote the new pack README so it gives task-specific `shape_refinement` guidance instead of the older proximal-boundary language.

Result:

- `pair_0120` and `pair_0043` are no longer active blockers for the side route.
- The new top-off pack is smaller and lower-friction than reopening the entire v2 batch.
- Replacement pair stats:
  - `pair_0208`: `bootstrap_mask_ratio 0.4796`, `aligned_luma_delta -0.0006`
  - `pair_0022`: `bootstrap_mask_ratio 0.2066`, `aligned_luma_delta +0.0010`

Conclusion:

- The shape-refinement side route should keep the current merged v2 approved subset as-is and continue expansion through the new `v3` top-off pack instead of revisiting `pair_0120` / `pair_0043`.
- The next manual boundary is now clean again: annotate `pair_0208` and `pair_0022` from the new `masked_shape_refinement_v3_seed` pack.

### Experiment 2026-04-23A - Archive And Evaluate The Second GPU-Side Shape-Refinement Pilot

Hypothesis:
If the expanded `shape_refinement_v3` subset really reduced the tiny-set wobble seen in the first pilot, then the second GPU-side pilot should still train stably and its checkpoint ranking should remain readable under patched local validation.

Change made:

- Archived the new Colab run into:
...
- Reviewed checkpoints, previews, metrics, and config for the `25 / 50 / 75 / 100` ladder.
- Re-ran local validation on all four checkpoints with `--disable-safety-checker` so `pair_0073` no longer blacks out:
...
- The run is stable and complete:
  - checkpoints at `25 / 50 / 75 / 100`
  - previews at `25 / 50 / 75 / 100`
  - no collapse or obvious late-run instability
- Unlike the first `shape_refinement` pilot, the patched validation ranking improves monotonically through the ladder:
...
- Preserve remains exact outside the mask under the patched protocol:
  - mean unmasked L1 to input `0.0`
  - mean unmasked deltaE to input `0.0`

Conclusion:

- The second GPU-side `shape_refinement` pilot is a cleaner success than the first one:
  - route is still stable
  - the expanded dataset reduced the earlier tiny-set wobble
  - current best checkpoint is `step100`
- The side route still benefits more from more clean shape examples than from nearby hyperparameter tuning.
- The next manual boundary should therefore be another small `shape_refinement` annotation batch rather than another training-only tweak.

### Experiment 2026-04-23B - Prepare The Next Clean Shape-Refinement Annotation Pack

Hypothesis:
If the second pilot remains stable and reaches its best checkpoint at the end of the current budget, then the highest-value next move is another small annotation expansion made of clean, defendable shape examples rather than offset-prone tails.
...
- Reserved the next final mask root:
...
- Selected the new seed split:
  - train: `pair_0028`, `pair_0057`, `pair_0106`, `pair_0158`
  - val: `pair_0035`
  - reserve val candidate: `pair_0216`
- Rewrote the pack README for task-specific `shape_refinement` guidance.

Result:

- The new batch keeps the route away from the retired offset-heavy tails and emphasizes long-french / distal-sidewall / tip-transition examples.
- Bootstrap stats are in a usable range for human cleanup:
  - `pair_0028`: bootstrap mask ratio `0.0988`
  - `pair_0057`: bootstrap mask ratio `0.2264`
  - `pair_0106`: bootstrap mask ratio `0.1376`
  - `pair_0158`: bootstrap mask ratio `0.2574`
  - `pair_0035`: bootstrap mask ratio `0.1280`

Conclusion:

- The next manual boundary is now explicit and local again: annotate the `masked_shape_refinement_v4_seed` pack.
- `pair_0216` should remain a reserve validation option and stay outside the active pack unless we later want a more conservative replacement val anchor.
